...
function unwrapApiEnvelope<T>(data: unknown) {
  ...
  if (!Number.isNaN(successCode) && successCode !== 0) {
    throw new ApiError(
      extractErrorMessage(data) ?? `Request failed with business code ${String(data.code)}`,
      200,
      data,
    );
  }
...
export async function apiRequest<T>(path: string, options: ApiRequestOptions = {}): Promise<T> {
  ...
  if (!response.ok) {
    throw new ApiError(
      extractErrorMessage(responseData) ?? `Request failed with status ${response.status}`,
      response.status,
      responseData,
    );
  }
