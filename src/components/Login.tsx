  ...
  const organizationHelperText = isLoadingOrganizations
    ...
    : selectedEmployee
      ? availableOrganizations.length > 0
        ? `将默认进入 ${selectedOrganization?.title ?? availableOrganizations[0]?.title ?? '当前帐套'}，登录后可在工作台切换`
        : '未配置帐套权限，请联系管理员'
      : '选择登录人员后，系统会自动匹配可进入的帐套';
        ...
        <div className="flex w-full max-w-6xl flex-col items-center gap-16 lg:flex-row">
          ...
          <div className="glass-card w-full max-w-md rounded-3xl p-8 md:p-10">
            ...
            <form className="space-y-5" onSubmit={handleSubmit}>
              ...
              <div className={`rounded-[24px] border px-4 py-3 backdrop-blur-xl ${
                selectedEmployee && availableOrganizations.length > 0
                  ? 'border-primary/10 bg-white/55 text-slate-600'
                ...
                <div className="flex items-start gap-3">
                  <div className={`mt-0.5 flex h-9 w-9 items-center justify-center rounded-2xl ${
                    ...
                    <span className="material-symbols-outlined text-[18px]">
                      {selectedEmployee && availableOrganizations.length > 0 ? 'apartment' : 'warning'}
                    </span>
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="text-[11px] font-bold uppercase tracking-[0.2em] text-slate-400">帐套入口</div>
