...
export function FieldInspectorController({
  ...
  const filledCompactCardClass = shadcnInspectorSectionClass;
  const inspectorOptionCardClass =
    'flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200/60 bg-white/80 px-3.5 py-3 transition-all backdrop-blur-md hover:border-[color:var(--workspace-accent-soft)] hover:bg-white hover:shadow-sm dark:border-slate-800 dark:bg-slate-900/80 dark:hover:bg-slate-900/92';
  const inspectorReadonlyFieldClass = `${fieldClass} cursor-default bg-white/80 text-slate-600 backdrop-blur-md dark:bg-slate-900/80`;
                ...
                {isBillHeaderField ? (
                  <>
                    ...
                    <div className="grid gap-3">
                      <label className={inspectorOptionCardClass}>
                        <input
                          type="checkbox"
                    ...
                    {billHeaderPositionItems.map((item) => (
                      <div key={item.key}>
                        ...
                        <input
                          ...
                          value={`${item.value}px`}
                          readOnly
                          className={inspectorReadonlyFieldClass}
                        />
                      </div>
                    ))}
                  </div>
              ...
              <section className={filledCompactCardClass}>
                ...
                <div className={`grid gap-3 ${isBillScopedField ? '' : 'sm:grid-cols-2'}`}>
                  {commonPropertySwitches.map((item) => (
                    <label
                      key={item.key}
                      className={inspectorOptionCardClass}
                    >
                      <input
                        type="checkbox"
                        className="mt-0.5 size-4 rounded border-slate-300 text-primary focus:ring-primary"
