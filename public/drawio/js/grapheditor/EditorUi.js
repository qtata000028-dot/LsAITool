...
/**
 * Enables the custom function design mode in the format panel.
 */
EditorUi.prototype.functionDesignMode = false;

/**
 * Stores module options for function design bindings.
 */
EditorUi.prototype.functionFlowModuleOptions = [];

/**
 * Stores the current function design module search keyword.
 */
EditorUi.prototype.functionFlowModuleQuery = '';

/**
 * Stores loaded function-flow module metadata keyed by module code.
 */
EditorUi.prototype.functionFlowModuleMetaByCode = {};

/**
 * Stores field control type options for function-flow fields.
 */
EditorUi.prototype.functionFlowFieldSqlTagOptions = [];

/**
 * Stores the latest preview artifacts for the current function-flow graph.
 */
EditorUi.prototype.functionFlowPreviewData = null;
...
EditorUi.prototype.installShapePicker = function()
{
	...
	graph.dblClick = function(evt, cell)
	{
		if (this.isEnabled())
		{
			...
			{
				if (ui.functionDesignMode === true && cell != null && this.getModel().isVertex(cell))
				{
					var incomingEdgeCount = 0;

					for (var i = 0; i < this.getModel().getEdgeCount(cell); i++)
					{
						var edge = this.getModel().getEdgeAt(cell, i);
					...
					{
						ui.postFunctionFlowEvent('functionFlowTargetDblClick', {
							cell: ui.getFunctionFlowCellPayload(cell),
							functionDesignMode: true
						});
						mxEvent.consume(evt);
						return;
					}
...
/**
 * Toggles the custom function design mode for the format sidebar.
 */
EditorUi.prototype.setFunctionDesignMode = function(value)
{
	value = value === true;

	if (this.functionDesignMode != value)
	{
		this.functionDesignMode = value;
	}
	...
	this.fireEvent(new mxEventObject('functionDesignModeChanged'));
};

EditorUi.prototype.postFunctionFlowEvent = function(eventName, payload)
{
	...
	var target = this.embedMessageSource || window.opener || window.parent;
	var message = Object.assign({ event: eventName }, payload || {});
	target.postMessage(JSON.stringify(message), '*');
};

EditorUi.prototype.getFunctionFlowCellAttributes = function(cell)
{
	var attrs = {};

	if (cell != null && mxUtils.isNode(cell.value) && cell.value.attributes != null)
	{
		for (var i = 0; i < cell.value.attributes.length; i++)
		{
			var attr = cell.value.attributes[i];
...
EditorUi.prototype.getFunctionFlowSelectionPayload = function()
{
	var graph = this.editor.graph;
	var cell = graph.getSelectionCell();
	...
	return this.getFunctionFlowCellPayload(cell);
};

EditorUi.prototype.getFunctionFlowCellPayload = function(cell)
{
	var graph = this.editor.graph;
	...
	return {
		attributes: this.getFunctionFlowCellAttributes(cell),
		id: cell.id,
		isEdge: graph.getModel().isEdge(cell),
		isVertex: graph.getModel().isVertex(cell),
		label: graph.convertValueToString(cell),
		style: cell.style || ''
	};
};

EditorUi.prototype.parseFunctionFlowJsonAttr = function(value, fallback)
{
	if (value == null || value === '')
	{
		return fallback;
...
EditorUi.prototype.getFunctionFlowGraphJson = function()
{
	var graph = this.editor.graph;
	var model = graph.getModel();
	var parent = model.getChildAt(model.getRoot(), 0);
	...
	for (var i = 0; i < model.getChildCount(parent); i++)
	{
		...
		var attrs = this.getFunctionFlowCellAttributes(cell);

		if (model.isVertex(cell))
		{
			var nodeMappings = this.parseFunctionFlowJsonAttr(attrs.ffFieldMappings, []);
			for (var m = 0; m < nodeMappings.length; m++)
			{
				var mapping = nodeMappings[m] || {};
				fieldMappings.push({
					aggregateFunc: mapping.aggregateFunc || '',
					exprText: mapping.exprText || '',
					exprType: mapping.exprType || '',
					id: mapping.id || (cell.id + '_mapping_' + m),
					isGroupBy: mapping.isGroupBy === true,
					isRequired: mapping.isRequired === true,
					isVisible: mapping.isVisible !== false,
					mappingScope: mapping.mappingScope || '',
					orderNo: (mapping.orderNo != null) ? mapping.orderNo : (m + 1),
					ownerCellId: cell.id,
					sourceCellId: mapping.sourceCellId || '',
					sourceField: mapping.sourceField || '',
					sourceTableScope: mapping.sourceTableScope || '',
					targetField: mapping.targetField || '',
					targetLabel: mapping.targetLabel || '',
					width: (mapping.width != null) ? mapping.width : 120
				});
			}

			nodes.push({
				cellId: cell.id,
				condition: attrs.ffCondition || '',
				defaultDetailAlias: attrs.ffDefaultDetailAlias || '',
				defaultMainAlias: attrs.ffDefaultMainAlias || '',
				detailFkToMain: attrs.ffDetailFkToMain || '',
				detailPk: attrs.ffDetailPk || '',
				detailSql: attrs.ffDetailSql || '',
				detailTable: attrs.ffDetailTable || '',
				mainBillNoField: attrs.ffMainBillNoField || '',
				mainPk: attrs.ffMainPk || '',
				mainTable: attrs.ffMainTable || '',
				moduleCode: attrs.ffModuleCode || '',
				moduleKind: attrs.ffModuleKind || '',
				moduleName: attrs.ffModuleTitle || graph.convertValueToString(cell) || '',
				primarySql: attrs.ffPrimarySql || ''
			});
		}
		else if (model.isEdge(cell))
		{
			var passFields = this.parseFunctionFlowJsonAttr(attrs.ffPassFields, []);
			edges.push({
				bridgeFromField: attrs.ffBridgeFromField || '',
				bridgeTable: attrs.ffBridgeTable || '',
				bridgeToField: attrs.ffBridgeToField || '',
				cardinality: attrs.ffCardinality || '',
				cellId: cell.id,
				detailLinkMode: attrs.ffDetailLinkMode || '',
				fromField: attrs.ffFromField || '',
				joinType: attrs.ffJoinType || '',
				passFields: passFields,
				relationCode: attrs.ffRelationCode || '',
				sourceCellId: cell.source != null ? cell.source.id : '',
				targetCellId: cell.target != null ? cell.target.id : '',
				toField: attrs.ffToField || ''
			});
		}
	}

	var preview = this.functionFlowPreviewData || {};
...
EditorUi.prototype.notifyFunctionFlowGraphChange = function()
{
	this.postFunctionFlowEvent('functionFlowGraphChange', {
		cell: this.getFunctionFlowSelectionPayload(),
		functionDesignMode: this.functionDesignMode === true,
		graphJson: this.getFunctionFlowGraphJson()
	});
};

EditorUi.prototype.setFunctionFlowPreviewData = function(preview)
{
	this.functionFlowPreviewData = preview || null;

	if (this.format != null && this.functionDesignMode)
	{
		this.format.refresh();
	}

	this.postFunctionFlowEvent('functionFlowPreviewChange', {
		preview: preview || null
	});
};

EditorUi.prototype.requestFunctionFlowModuleMeta = function(moduleCode, cellId)
{
	...
	this.postFunctionFlowEvent('functionFlowModuleMetaRequest', {
		cellId: cellId || null,
