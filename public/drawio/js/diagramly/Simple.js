...
(function()
{
	...
	function ensureFunctionModeSwitch(ui)
	{
		if (ui.functionModeSwitchElt == null)
		{
			ui.functionModeSwitchElt = document.createElement('div');
			ui.functionModeSwitchElt.className = 'geFunctionModeSwitch';

			ui.styleDesignElt = document.createElement('button');
			ui.styleDesignElt.className = 'geBtn geFunctionModeBtn';
			ui.styleDesignElt.setAttribute('type', 'button');
			ui.styleDesignElt.innerText = '样式设计';

			ui.functionDesignElt = document.createElement('button');
			ui.functionDesignElt.className = 'geBtn geFunctionModeBtn';
			ui.functionDesignElt.setAttribute('type', 'button');
			ui.functionDesignElt.innerText = '功能设计';

				mxEvent.addListener(ui.styleDesignElt, 'click', mxUtils.bind(ui, function()
				{
					this.setFunctionDesignMode(false);

					if (this.toggleFormatPanel != null && !this.isFormatPanelVisible())
					{
						this.toggleFormatPanel(true);
					}
				}));

				mxEvent.addListener(ui.functionDesignElt, 'click', mxUtils.bind(ui, function()
				{
					this.setFunctionDesignMode(true);
			...
			ui.functionModeSwitchElt.appendChild(ui.styleDesignElt);
			ui.functionModeSwitchElt.appendChild(ui.functionDesignElt);

			var updateFunctionModeSwitch = mxUtils.bind(ui, function()
			{
				if (this.styleDesignElt != null)
				{
					this.styleDesignElt.classList.toggle('geActive', !this.functionDesignMode);
				}

				if (this.functionDesignElt != null)
				{
					this.functionDesignElt.classList.toggle('geActive', this.functionDesignMode);
				}
			});

			ui.addListener('functionDesignModeChanged', updateFunctionModeSwitch);
			updateFunctionModeSwitch();
		}

		return ui.functionModeSwitchElt;
	}
		...
		if (value == 'simple' || value == 'sketch')
		{
			if (this.sketchFooterMenuElt == null)
			{
				...
				if (value != 'simple')
				{
					...
					if (value == 'simple')
					{
						var functionModeSwitchElt = ensureFunctionModeSwitch(this);
						functionModeSwitchElt.style.flexShrink = '0';
						functionModeSwitchElt.style.marginLeft = '10px';
						functionModeSwitchElt.style.marginRight = '6px';
						footer.appendChild(functionModeSwitchElt);
					}
			...
			if (this.sketchMenubarElt == null)
			{
				...
				this.dependsOnLanguage(mxUtils.bind(this, function()
				{
					...
					if (this.formatElt != null)
					{
						...
						if (this.exitElt != null)
						{
							this.exitElt.setAttribute('title', mxResources.get('exit'));
						}

						if (this.styleDesignElt != null)
						{
							this.styleDesignElt.setAttribute('title', '切换到样式设计');
						}

						if (this.functionDesignElt != null)
						{
							this.functionDesignElt.setAttribute('title', '切换到功能设计');
						}

						if (this.overflowMenuElt != null)
						{
							this.overflowMenuElt.setAttribute('title', mxResources.get('more'));
						}
				}));
			}
				...
				if (value == 'simple')
				{
					...
					{
						var embedButtons = this.buttonContainer.querySelectorAll('.geEmbedBtn');

						for (var i = 0; i < embedButtons.length; i++)
						{
							if (embedButtons[i].parentNode == this.buttonContainer)
							{
								this.buttonContainer.removeChild(embedButtons[i]);
							}
						}
				...
				if (this.mainMenuElt == null)
					...
					if (this.formatElt == null)
					{
						this.formatElt = this.createMenuItem('format', Editor.dockRightImage, true);
					...
					this.formatElt.style.backgroundImage = 'url(' + Editor.dockRightImage + ')';
					this.formatElt.style.backgroundSize = '24px';
					this.formatElt.style.flexShrink = '0';
					this.formatElt.style.marginLeft = '0';
					this.sketchMenubarElt.appendChild(this.formatElt);

					if (this.overflowMenuElt == null)
					{
						this.overflowMenuElt = this.createMenu('dynamicAppearance', Editor.gearImage);
					...
					this.overflowMenuElt.style.backgroundImage = 'url(' + Editor.gearImage + ')';
					this.overflowMenuElt.style.backgroundSize = '22px';
					this.sketchMenubarElt.appendChild(this.overflowMenuElt);

					if (urlParams['embed'] == '1' && urlParams['noExitBtn'] != '1')
					{
						if (this.exitElt == null)
						{
							this.exitElt = this.createMenuItem('exit', Editor.crossImage, true);
						}

						this.exitElt.style.flexShrink = '0';
						this.exitElt.style.marginLeft = '0';
						this.exitElt.style.backgroundSize = '18px';
						this.sketchMenubarElt.appendChild(this.exitElt);
					}
				}
