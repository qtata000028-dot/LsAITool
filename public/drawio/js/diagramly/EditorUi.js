...
//		}
	...
	EditorUi.prototype.installMessageHandler = function(fn)
	{
		...
		var sendSelectionChange = mxUtils.bind(this, function()
		{
			...
			this.postFunctionFlowEvent('selectionChange', {
				cell: this.getFunctionFlowSelectionPayload(),
				functionDesignMode: this.functionDesignMode === true
			});
		});
		
		var sendGraphChange = mxUtils.bind(this, function()
		{
			this.notifyFunctionFlowGraphChange();
		});
		...
		this.editor.graph.getModel().addListener(mxEvent.CHANGE, sendGraphChange);
		...
		mxEvent.addListener(window, 'message', mxUtils.bind(this, function(evt)
		{
			...
			if (urlParams['proto'] == 'json')
			{
				...
				try
				{
					...
					else if (data.action == 'toggleFormatPanel')
					{
						this.toggleFormatPanel(data.visible);
						sendSelectionChange();

						return;
					}
					else if (data.action == 'functionFlowConfig')
					{
						this.functionFlowModuleOptions = (data.moduleOptions != null) ? data.moduleOptions : [];
						this.functionFlowFieldSqlTagOptions = Array.isArray(data.fieldSqlTagOptions) ? data.fieldSqlTagOptions : [];
						var applyFunctionFlowConfig = mxUtils.bind(this, function()
						{
							if (data.mode != null)
							{
								this.setFunctionDesignMode(data.mode === 'function');
							}
							...
							if (data.mode === 'function' && this.toggleFormatPanel != null && !this.isFormatPanelVisible())
							{
								this.toggleFormatPanel(true);
							}
						...
						applyFunctionFlowConfig();
						window.setTimeout(applyFunctionFlowConfig, 0);

						return;
					}
						else if (data.action == 'functionFlowModuleMeta')
						{
							if (data.moduleCode != null && data.meta != null)
							{
								this.functionFlowModuleMetaByCode[data.moduleCode] = data.meta;

							if (data.cellId != null)
							{
								var graph = this.editor.graph;
								var targetCell = graph.getModel().getCell(data.cellId);

									if (targetCell != null)
									{
										var meta = data.meta;
										var fields = Array.isArray(meta.fields) ? meta.fields : [];
										var normalizedFieldName = function(value)
										{
											return (value || '').toString().toLowerCase().replace(/[\s_\-]/g, '');
										};
										var pickSuggestedField = function(candidates, scope)
										{
											var preferred = [];
											var fallback = [];
											...
											for (var fi = 0; fi < fields.length; fi++)
											{
												...
												var haystack = normalizedFieldName([
													field.fieldName,
													field.fieldLabel,
													field.fieldKey,
													field.systemName
												...
												{
													var candidate = normalizedFieldName(candidates[ci]);
													if (candidate === '')
													{
														continue;
													}
													if (haystack === candidate || haystack.indexOf(candidate) >= 0)
													{
														preferred.push(field.fieldName || '');
														break;
													}
													if (candidate.indexOf(haystack) >= 0)
													{
														fallback.push(field.fieldName || '');
														break;
													}
												}
											}

											return (preferred.length > 0) ? preferred[0] : ((fallback.length > 0) ? fallback[0] : null);
										};
										graph.getModel().beginUpdate();

										try
										{
										var applyIfBlank = mxUtils.bind(this, function(name, value)
										{
											if (!value)
											{
												return;
											...
											var currentValue = graph.getAttributeForCell(targetCell, name, '');
											if (currentValue == null || currentValue === '')
											{
												graph.setAttributeForCell(targetCell, name, value);
											}
										});

										applyIfBlank('ffModuleKind', meta.moduleKind || null);
										applyIfBlank('ffMainTable', meta.mainTable || null);
										applyIfBlank('ffDetailTable', meta.detailTable || null);
											applyIfBlank('ffPrimarySql', meta.mainSql || null);
											applyIfBlank('ffDetailSql', meta.detailSql || null);
											applyIfBlank('ffFormKey', meta.formKey || null);
											applyIfBlank('ffCondKey', meta.condKey || null);
											applyIfBlank('ffDefaultMainAlias', 'a');
											applyIfBlank('ffDefaultDetailAlias', 'b');
											applyIfBlank('ffMainPk', pickSuggestedField(['id', 'billid', 'fid', '单据id'], 'main'));
											applyIfBlank('ffMainBillNoField', pickSuggestedField(['billno', 'billcode', 'bill_code', 'djh', '单号'], 'main'));
											applyIfBlank('ffDetailPk', pickSuggestedField(['detailid', 'mxid', 'detail_id', 'mx_id'], 'detail'));
											applyIfBlank('ffDetailFkToMain', pickSuggestedField(['billid', 'mainid', 'fid', 'bill_id', 'main_id'], 'detail'));
										}
										finally
										{
											graph.getModel().endUpdate();
					...
					else if (data.action == 'functionFlowPreviewResult')
					{
						this.setFunctionFlowPreviewData(data.preview || null);
						sendSelectionChange();

						return;
					}
					else if (data.action == 'functionFlowRequestSelection')
					{
						sendSelectionChange();

						return;
					}
					else if (data.action == 'functionFlowSetCellData')
					{
						var graph = this.editor.graph;
						var cell = (data.cellId != null) ? graph.getModel().getCell(data.cellId) :
						...
						if (cell != null)
						{
							...
							try
							{
								if (data.attributes != null)
								{
									for (var key in data.attributes)
									{
										if (data.attributes.hasOwnProperty(key))
										{
											...
											graph.setAttributeForCell(cell, key,
								...
								if (data.label != null)
								{
									graph.cellLabelChanged(cell, data.label, false);
								}
							}
							finally
							{
	...
	EditorUi.prototype.addEmbedButtons = function()
	{
		this.dependsOnLanguage(mxUtils.bind(this, function()
		{
			...
			var simpleEmbedTheme = Editor.currentTheme == 'simple';
			...
			else
			{
				...
				if (urlParams['noExitBtn'] != '1' && !simpleEmbedTheme)
				{
					this.embedExitButton = this.createEmbedButton(urlParams['publishClose'] == '1' ?
						mxResources.get('close') : mxResources.get('exit'),
						this.actions.get('exit').funct);

					this.buttonContainer.appendChild(this.embedExitButton);
				}
				else
				{
					this.embedExitButton = null;
				}
		}));
	};
