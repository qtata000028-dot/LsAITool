	...
	if (this.menubar != null)
	{
		...
		mxEvent.preventDefault(toggleFormatElement);
			...
			var toggleFormatPanel = mxUtils.bind(this, function()
			{
				...
				{
					toggleFormatElement.classList.add('geActiveItem');
				}
				else
				{
					toggleFormatElement.classList.remove('geActiveItem');
				}
			});

			toggleFormatElement.style.backgroundImage = 'url(\'' + Editor.dockRightImage + '\')';
			toggleFormatElement.style.backgroundSize = '24px';
		...
		if (urlParams['embed'] != '1' && Editor.currentTheme != 'atlas')
		{
			...
			this.dependsOnLanguage(mxUtils.bind(this, function()
			{
				...
				toggleFormatElement.setAttribute('title', '切换右侧面板');
			}));
		}
